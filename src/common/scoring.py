...
4개 축 가중합 (최대 100):
  ...
  - 시설 적합성 (0~30): 자이씨앤에이 시공영역(반도체CR·GMP바이오·이차전지) 부합도.
  - 투자 규모 (0~20): 클수록 큰 발주 기회. 미공시는 중간값.
...
2026-06-05 재조정: 시설 적합성 20→30, 투자 규모 30→20 으로 스왑.
금액이 시설적합도를 덮어써, 시공영역 아닌 '기타' 초대형 공급계약(다원시스 2,151억 등)이
진짜 CR/바이오 리드를 TOP 순위에서 누르던 문제 교정. 자이는 시공사 — '무엇을 짓느냐'가
'얼마짜리냐'보다 우선이다.
"""
...
# ── 시설 적합성 (0~30) — categorize.py 카테고리명 기준 ──
FACILITY_SCORE = {
    "CR": 30, "제약/바이오": 30, "이차전지": 30,   # 핵심 시공영역
    "데이터센터": 27,                               # 첨단 산업시설 (클린전원·냉각 시공)
    "일반생산": 18,
    "식품/음료": 12, "R&D": 12, "화장품": 12,
    "기타": 0,                                      # 자이 시공영역 아님 — 금액 커도 적합도 0
}
...
def _scale_score(amount_won: int) -> int:
    """투자규모 점수 (0~20). 미공시(0)는 9(중간 — 규모 미상이 작다는 뜻은 아님).

    2026-06-05: 금액 가중 30→20 으로 하향(시설적합성과 스왑). 금액이 적합도를 덮어쓰던
    문제 교정 — 구간 비율은 유지하고 상한만 낮췄다.
    """
