...
### 예약 드롭 대비 (2026-08-28 확립)
- GitHub 이 `schedule` 이벤트를 **아예 발생시키지 않는** 경우가 있다(고부하 시 드롭).
  run 이 없으면 실패 알림도 없다 — 2026-08-28 목 21:40 UTC 슬롯이 통째로 사라져
  주간 발송이 조용히 누락됐다 (직전 며칠 GitHub Actions 장애 다발).
- **금요일 오전 안에 끝나는 타임라인** (영업팀은 금요일 오전에 받아야 한다):
  | 시각(KST) | 무엇 |
  |---|---|
  | 금 06:40 / 07:40 / 09:40 | weekly 자동 시도 3회 (독립 예약 슬롯) |
  | 금 11:00 | daily-collect 의 발송 누락 감시 → 미발송이면 운영자 알림 |
  | 토 06:00 이후 매일 | 같은 감시 반복 (금 11:00 슬롯까지 드롭된 경우 대비) |
- 대응 1: 슬롯 3개 중 먼저 성공한 것만 발송.
  `scripts/ci_weekly_guard.py --mode guard` 가 '오늘 이미 발송 성공한 run' 을 찾아
  중복 발송을 막는다 (판정 근거 = 그 run 의 `Send email` 스텝 success).
  수동 dispatch 는 항상 진행(재발송 의도 존중), 판정 실패 시에도 진행(누락 > 중복).
- 대응 2: `--mode watchdog` 은 `FRIDAY_DEADLINE_HOUR`(11시) 이후에만 그날을 대상으로 본다.
  아직 진행 중인 weekly run 이 있으면 알림을 보류한다(허위 경보 방지).
- 한계: 감시 자체도 GitHub 예약이라 스케줄러 장애 창이 길면 같이 드롭될 수 있다.
  완전 독립 트리거가 필요하면 사내 PC 작업 스케줄러에서 API dispatch 하는 방법뿐.
